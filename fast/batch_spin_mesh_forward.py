...
def spin_and_render_single(mesh_path, out_dir, renderer,
                           n_frames=60, width=800, height=800,
                           axis="y", frame_index=0, step=0.0):
    """
    Spin a single mesh and save n_frames images into out_dir.
    Axis can be 'x', 'y', or 'z'.
    frame_index: index of this GLB in the walking sequence.
    step: how far to move forward per GLB, along +z.
    """
    ...
    # Center + scale to unit size
    base_tm.apply_translation(-base_tm.centroid)
    scale = 1.0 / max(base_tm.extents)
    base_tm.apply_scale(scale)

    # Move this frame forward in world space
    if step != 0.0:
        T = np.eye(4)
        T[:3, 3] = [0.0, 0.0, frame_index * step]  # move along +z
        base_tm.apply_transform(T)
    ...
    for i in range(n_frames):
        angle = 2.0 * np.pi * i / n_frames  # 0 -> 2π
...
def batch_spin(glb_dir, out_root,
               n_frames=60, width=800, height=800,
               axis="y", step=0.0):
    """
    step: forward movement per GLB (in world units).
    0.0 = stay in place; >0 = walk forward along +z.
    """
...
def main():
    ...
    ap.add_argument("--glb-dir", required=True, help="Directory containing .glb files")
    ap.add_argument("--out-dir", default="spins", help="Output root directory")
    ap.add_argument("--frames", type=int, default=60,
                    help="Number of spin frames per GLB")
    ap.add_argument("--width", type=int, default=800, help="Render width")
    ap.add_argument("--height", type=int, default=800, help="Render height")
    ap.add_argument("--axis", type=str, default="y",
                    choices=["x", "y", "z"], help="Axis to spin around")
    ap.add_argument("--step", type=float, default=0.0,
                    help="Forward movement per GLB along +z (0 = in place)")
    args = ap.parse_args()
